...
import ImportantReview from '@/views/ImportantReview/ImportantReview.vue'
import InspectionViewTabs from './components/InspectionViewTabs.vue'
...
const workbenchAiHistory = computed(() =>
  buildWorkbenchAiHistory({
    current: workbenchAiPresentation.value,
    projectAnalysis: projectAnalysisPresentation.value,
...
const {
  ...
  recordOpinionOverride: recordAiOpinionOverride
} = useAiFindingFeedback({
  runId: () =>
    String(
      (workbenchAiPresentation.value.sourceLabel === '节点 AI 复核'
        ? aiRecheckDisplayRun.value?.id
        : latestAiRun.value?.id) || ''
    ),
...
const reviewConclusionOverall = computed(() => {
  if (projectAnalysisIsDisplayed.value) return workbenchAiPresentation.value.summary
...
const handleInspectionWorkspaceViewChange = async (view: InspectionWorkspaceView) => {
  ...
  activeWorkbenchSection.value = view !== 'list' || keepNode ? 'node' : 'overview'
  await updateInspectionRoute(
    activeWorkbenchSection.value === 'overview'
      ? { overview: true }
      : { nodeId: activeNodeId.value, auditItem: activeInspectionAuditItem.value },
...
const handleSaveReviewOpinion = async () => {
  ...
  const aiSuggested =
    latestAiRun.value && !latestAiReviewDisplay.value.failed
      ? String(latestAiRun.value.suggestion?.result || '')
      : ''
  const aiMapped = aiSuggested ? AI_RESULT_TO_OPINION[aiSuggested] : undefined
  const overrideReason =
    aiMapped && aiMapped !== reviewResult.value
      ? await askAiOverrideReason(aiSuggested, reviewResult.value)
      : undefined
  if (aiMapped && aiMapped !== reviewResult.value && !overrideReason) return
  actionLoading.value = true
  try {
    ...
    if (overrideReason)
      await recordAiOpinionOverride(overrideReason, aiSuggested, reviewResult.value)
    await loadProjectBundle()
  } finally {
    actionLoading.value = false
  }
      ...
      <header class="topbar">
        ...
        <div class="top-actions">
          ...
          <InspectionViewTabs
            ...
            :model-value="activeInspectionWorkspaceView"
            @change="handleInspectionWorkspaceViewChange"
          />
          <AutoReviewControl
            v-if="role === 'inspection'"
        ...
        :class="[
          'workspace',
          {
            'no-left-nav':
              role === 'contractor' ||
              role === 'ndt' ||
              activeInspectionWorkspaceView === 'important',
            'is-left-collapsed':
...
.center.has-flush-audit-directory > .inspection-ai-review-region,
.center.has-flush-audit-directory > .important-review {
