const descriptions: Record<string, string> = {
  signature_authenticity_and_authority:
    '请核对签名是否有效、签字人是否有相应权限，系统目前只能核对签批记录。',
  owner_reply_validity_and_timing:
    '请核对建设单位批复是否有效、对应哪个方案版本，以及批复时间是否满足要求。',
...
export const automationLimitationScope = (atomicCheckId?: string): string => {
  ...
  if (!match || Number(match[1]) < 1 || Number(match[1]) > 69 || Number(match[2]) < 1) return ''
