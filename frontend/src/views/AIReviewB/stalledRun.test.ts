...
const active = sfc.slice(
  sfc.indexOf('const executionActive'),
  sfc.indexOf('const showExecutionActivity')
)
...
// 摘要要给出停在哪一刻 + 下一步能做什么
const summary = sfc.slice(
  sfc.indexOf('const executionSummary'),
  sfc.indexOf('const executionStatusLabel')
)
...
assert.ok(/updatedAt \|\| activeRun\.value\?\.createdAt/.test(sfc), '停滞时间应以最后更新为准')
