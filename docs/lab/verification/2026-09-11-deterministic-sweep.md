...
## 十六、焊接重点优化与界面生码清零（2026-09-13）

### 焊接：平台代号进事实链，并挡住张冠李戴

节点 24 实测链条（每一步都是线上跑出来的）：

| 阶段 | all_codes_decoded | 说明 |
|---|---|---|
| 起点 | 0/4 | OCR 把代号认成 CTAF/SHAW/PTAV，解码器全拒；平台核到的原文只进了 certificate_facts，r24 的焊工事实是另一个 builder 从 OCR 建的，拿不到 |
| 接上平台 | 4/5 | **其中四个是别人的**——平台按李卫伍的证件号返回的持证人是「赵相军」，系统照抄 |
| 加姓名核对 | 0/4 | 姓名不符判 verified_mismatch，不覆盖不标已核验；姜军那次平台超时 |
| 平台恢复后 | **2/5** | 姜军 verified_match（登记原文，2 个代号解得开）；李卫伍 verified_mismatch（保留 OCR，解不开） |

`verified_mismatch` 是这轮最有价值的产出：证件号是 OCR 读的，错一位就查到别人，
照抄回来等于把另一个人的合格项目安到这名焊工头上，比查不到危险得多。模型也接住了
这条事实，写成发现「焊工证号与持证人姓名不一致……需人工现场核对证件原件」。

其余焊接侧修正：一个焊工一条证（OCR 把两人拆成 15 行，10 行是空模板）；同一张证被
r24 builder 与证书链各产一条，按证号去重；事实标签中文化（`r24-certificates-1` →
「焊工证 姜军」）；引文从 `json.dumps(row)` 改成「键：值」并滤掉 OCR 读坏的列名；
证据带 documentId，点文件名直接打开原件。
...
做法是按词翻译 + 后缀组框（`_missing`/`_not_applicable`/`_unresolved`/`_mismatch`…，
`_or_`/`_and_` 拼「或/与」），200 余个业务词的词典 + 不规则整段词根表；**认不出的词原样
保留**，不硬翻。两个守门测试（`reasonCodeCoverage`、`rawCodeCoverage`）从后端源码现抽
代号，后端新增一个词而前端没翻就红。

### 节点页信息密度

节点包返回 258KB，页面原来只渲染 2.2KB——「审查所需资料」表锁在 submission 审计项下，
而节点页目录只保留 ai_review / human_review 两项，那张表永远打不开。现在：七个审计项的
状态条（含 OCR/证据项的问题摘要）、本节点所需资料卡（必传 x/y、缺什么、谁该传、去挂接）、
每个原子项的原因与逐条检查、按类型归组折叠的事实与证据、认不出可核字段的表格行汇总成
一句并指到原件。

### 运维

同一台服务器磁盘 98% 满导致一次部署失败（`No space left on device`）。清掉四个旧测试
工作区与旧 dist 包腾出 4.7GB；大头是另一个项目的 15 个历史镜像（约 15GB），未动。
另有两次部署因 SSH 断线失败（`Connection closed by UNKNOWN port 65535`），重试即过。

## 十七、浏览器线上审计（2026-09-13）

用监检账号在生产上逐节点巡检，抓到的**不只是显示问题**：

| # | 发现 | 性质 |
|---|---|---|
| 1 | 事实标签 `design_license TS1844171-2028` | 生码；后端中文化 + 前端兜底老留痕 |
| 2 | f-string 拼的检查码（`holder_matches_registry`…）**整类逃过守门测试** | 守门正则只认字面量 |
| 3 | 带序号检查码 117 个（`component_1_batch_traceable`） | 组合翻译「元件 1：批号可追溯」 |
| 4 | **检查项的期望值也是代号** | 之前只翻码不翻值；`_and_` 还会把整词切碎（第一版没修对） |
| 5 | 一次失败的复核把上一次好结果挤掉 | 产品缺陷：回退显示上一次成功结果 + 失败横幅 |
| 6 | 空表格行变成事实 | r24–r34 全节点，源头砍掉 |
| 7 | 27 个复核擱淺在 queued 无人回收 | 生产数据问题 |
...
- **磁盘撑爆**：一次发起 30 个复核 + 反复部署，Docker build cache 堆到 44GB，99G 盘满 →
  Postgres 进入恢复 → API 短暂 not_ready。清理后恢复；此后批量复核改成每批 4 个，
  部署后顺手 `docker builder prune -f`。
- **两次把自己的测量错误当成产品 bug**：① 「节点 2 与节点 1 摘要相同」实为改 hash 不触发
  重新加载；② 「点左树无法切换节点」实为把视窗设成 1440 后仍按 800 的坐标点击，落点错位。
  用 DOM 直接 click 验证后切换完全正常。教训：先证伪自己的测量，再下产品结论。
