...
| 全量回歸 | **5263 通過 / 0 失敗** | 含 77 條 PostgreSQL 整合測試（當天才第一次真正執行） |
...
1. **模型密鑰無效**——`AICHECK_LLM_API_KEY` 是 `sk-ws-` 開頭 116 字元，阿里雲回 `invalid_api_key`；DeepSeek 回退已由項目方棄用。AI 審查、一鍵分析、文件分類自 2026-09-09 04:11 起全部不可用，無告警（已補 `scripts/model_reachability_probe.py`）。
2. **語料裡沒有施工記錄**——121 份文件是設計、許可、資格、質量證明、檢測報告；R44／R46／R47／R49／R50／R52～R55／R68 等施工類規則沒有可讀的紙。
3. **判據的生產側只接通兩個欄位**——三層機制（表結構分類、欄位對映、agent 讀正文填判斷）當天建成且離線可重放，但真實覆蓋仍是 1/73 與 2/465，且第三層未曾真實運行（卡在第 1 點）。
