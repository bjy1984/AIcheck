...
整體工程進度粗估：**約60%**（2026-09-09，五批次等權估算，非驗收通過率）。清單原型90%、工作台80%、操作閉環75%、規則整合驗收35%、品質／發布準備20%。主要缺口仍為業務覆蓋、真實案件與發布前置；未證明達90%。

## 2026-09-24 實測數字（PR #26–#38 合併後）

量法與 9 月 10 日相同，可逐項複核；「快照」指七項目唯讀快照（`aicheck-jev-seven-single-20260923`）。

| 量的東西 | 09-10 | 09-24 | 怎麼量／出處 |
|---|---|---|---|
| 規則可離線重放 | 47 / 69 | **47 / 69** | `compile_node_tool_plan` + `OFFLINE_TOOLS` |
| 判據欄位有生產者（簽名） | 2 / 465 | **14 / 476** | 規則包全部 requiredPaths／actualPath／applicabilityPath 對簽名欄位；新增 R47、R66、R67、R68 |
| 表結構簽名 | 1 | **7**（另 3 種按行文字版式） | `table_schema_signatures.yaml`：材料證明、靜電接地、吹掃、泄漏試驗×2、管道特性表、壓力管道匯總表 |
| 快照上的設計管線 | 1（亂字） | **23**（5 個項目） | `build_project_pipelines`；7F7270 2、6B15AE 1、OCR-002 1、ECD202 8＋1、OCR-001 8、GDLNG 2 |
| 讀得到的施工記錄表 | 0 | **4 張**（GDLNG 交工資料） | 靜電接地 A.0.10、壓力與泄漏試驗 A.0.16（豎排）、吹掃 A.0.17、壓力管道匯總表（豎排） |
| R35 以後的來源 | 全部「缺失或含糊」 | 按實際掛載讀取 | 原因是快照缺版本 tenantId（#27），生產不受影響 |
| 適用性可判的設計依據 | 無 | ECD202／OCR-001 8 條管段 | 介質特性「可燃」→R47、泄漏試驗要求「√」→R66／R67（#38） |
| 後端全套 | 5,305 通過 | **5,765 通過 / 0 失敗** | 本機（81 條 PostgreSQL 整合測試跳過）；ruff 286／286 |
| 前端單測 | 102 | **111 通過 / 0 失敗** | `pnpm test:unit`；vue-tsc 通過 |

**規則結論的變化：**每一步都在快照上離線重放了全部 133 個可重放的工程×規則，結論**零變化**。原因不是沒效果，而是：多條管線的記錄要先由監檢員選定對象（#30）；適用性只宣告「適用」、不宣告「不適用」（#31）；ECD202 在 R47／R66／R67 上掛的是施工圖，還沒有接地、試壓記錄。

**本輪確認的業務口徑**（詳見 `docs/lab/2026-09-24-design-pipeline-columns-request.md` 與記錄適用性模組）：R68 有記錄即適用；R66／R67 看設計的泄漏試驗要求或介質毒性／泄漏危害；R47 看火災危險性或介質特性可燃；不做介質危害對照表；記錄表本可帶卻沒寫的欄位才判不符合，別的文件才有而未抽的判「未抽取」交人工；管線號尾碼「-100」是公稱直徑。

**仍然卡住的：**施工記錄只有 GDLNG 一本；其餘項目的設計表沒有危害欄位（已請設計方補，#35）；R40 缺檢測事件清單；R55 支吊架（一管多行）、R44 防腐（轉置）尚未能讀；瀏覽器驗收需可登入環境；main 的 CI 自 09-08 起為紅（#25 草稿）。

## 2026-09-10 實測數字（取代印象估算）

這個百分比一直沒有計算方式。下面幾項是當天量出來的，可以逐項複核：

| 量的東西 | 數 | 怎麼量的 |
|---|---|---|
| 規則可離線重放 | **47 / 69** | `compile_node_tool_plan` + `NDT_FACT_BUILDERS` + `OFFLINE_TOOLS` 逐條試；當天從 3 條提到 47 條 |
| 卡在非離線工具的規則 | **0** | 同上；已由 `tests/test_offline_replay_coverage.py` 釘住不再回退 |
| 判據欄位有生產者 | **2 / 465** | 從規則包抽出全部 requiredPaths／actualPath／applicabilityPath |
| 表結構有簽名 | **1 / 73** | fact builder 讀的 businessSchema 名 vs `table_schema_signatures.yaml` |
| 語料裡的施工記錄 | **0 / 121** | 生產庫三個真實工程的文件檔名與物料類型 |
| 全量回歸 | **5305 通過 / 0 失敗** | 含 77 條 PostgreSQL 整合測試 |
| 帶凍結範圍的真實運行 | **1 / 330**（2026-09-11 起工位模式已開，新運行都會帶） | `RRUN-02721A7D01` |
| 真實驗收產物 | **1 / 276**（R43 · insufficient · 重放一致 · 未簽字） | `export_review_acceptance_fixture.py` |
| 前端單元測試 | **102 通過 / 0 失敗** | `pnpm test:unit`；`vue-tsc`／ESLint／Stylelint 同時通過 |
| 視覺稽核 | **32/32 路由、低對比 0** | `pnpm audit:ui`。修之前它報「低對比 0」但 32 條路由一條都沒打開——那是「沒測」不是「沒問題」 |

**重放上限不等於驗收進度。** 重放要有 fixture，而真實運行要能導出成 fixture，還需要凍結的文件範圍（生產目前 0 筆有）與真實素材。

### UI/UX 當天的實際變化

視覺稽核從「假的通過」變成真的全綠：原本非 strict 模式下永遠不會失敗，32 條路由
全部 `ERR_CONNECTION_REFUSED` 仍報低對比 0。修好判定後量到 **49 筆低對比**，逐條
修掉到 **0**：監檢工作台狀態標籤 2.45:1、施工方待處理膠囊 2.95:1、Element 的
錯誤／警告／info 文字 2.61～2.75:1。另修掉稽核自己等錯重定向路由的問題（4 條路由
一直量不到）。

**仍未做**：帶真實原文的完整案件驗收、規則發布與交接的視覺／鍵盤驗收——都要登入。
手機依使用者要求暫緩。

### 當天新增的三個攔路點（都不在程式裡）

1. ~~模型密鑰無效~~——2026-09-11 全部恢復：文本走 Token Plan，視覺與向量化走按量計費（`qwen-vl-max` / `text-embedding-v4` 1024 維），生產實測通過。
2. **語料裡沒有施工記錄**——121 份文件是設計、許可、資格、質量證明、檢測報告；R44／R46／R47／R49／R50／R52～R55／R68 等施工類規則沒有可讀的紙。
3. **判據的生產側只接通兩個欄位**——三層機制當天建成；2026-09-11 第三層已在真實文件上跑通（agent 對施工方清單如實回 null，反編造閘門有效），真實覆蓋仍是 1/73 與 2/465。
...
| 交接 | 草稿、來源檢查、核驗API；原工作台建立入口與授權接收任務查詢；PostgreSQL跨連線核驗衝突實測通過；新任務交接輸入／依賴失效與工作程序按需載入、前端選用／需重驗提示已接線；正常登入選用與新任務切換、協作草稿建立到接收端已驗 | 真實原文／失敗恢復／視覺與鍵盤驗收、執行中途來源變動的PostgreSQL檢查點已驗；兩個實際HTTP程序的核驗衝突、最新讀取與重啟還原已驗；執行結果提交交易邊界驗收 | 待接線／驗收 |
| 結果UI | 分層卡片、口語提示、完整清單、證據定位；已接入待辦首頁；空结果与执行状态分开，排队实机已验；完整工作台未執行誤顯結論及預填通過意見已修，登入分頁已驗；系统能力未完成独立提示已接线；R11 簽批逐項口語清單已接原首頁並完成合成瀏覽器／實際 API 回傳驗收，共用原文定位已補版本及頁碼矛盾阻擋 | 已登入種子工程選資料／人工保存已驗；仍需帶真實原文的完整案件及可用性驗收 | 待驗收 |
| 工位導航 | 已接原工作台桌面節點樹及手機抽屜，保留工程總覽計數；工位待審查／待確認／待補正統計及篩選件數已接入並登入實測 | 具來源的跨節點需重驗只讀API已補，原工位需重驗／未知篩選已接線，正常回傳／需重驗／未知瀏覽器流程已驗，跨工程競態／錯工程／503 重試元件驗收及 PostgreSQL 另一程序改寫／刪除已驗，完整多 HTTP 服務程序驗收待補；手機依使用者要求暫緩 | 待驗收／增補 |
...
| 其餘待驗收規則 | 既有工具與部分資料；R11-01簽批記錄、批復／採用時序及有來源的多事件清單及逐筆來源隔離已接線和合成驗收（效力與權限仍阻擋）；R11-02同對象參數與聲明對象清單全覆蓋比對已接線及合成案例驗收；R40事件記錄／報告對應、原文身份、明示參數／要求版本及逐事件結論比較已接線並合成驗收；真實 PQR 附件找到 RT 報告，尚缺合適原始記錄／唯一映射 | R10/R11/R40/R43–R58/R63–R64/R66–R68逐條缺口驗收 | 待核查／補齊 |
| 標準 | 已入庫資料及部分查詢修復；壓力倍率缺失／非法值與未知方法猜值已修，pressureTest 凍結標籤已對原頁校正，倍率比較不再先四捨五入，氣壓1.33倍上限已接入規則，倍率與絕對壓力矛盾及泄漏壓力誤讀已修 | 26查詢／原因清單未結項，14976／13296／47014 C–F剩餘資料 | 待補齊 |
| 驗收工具 | 發布門檻、R35–37重放、三組品質統計；2026-09-10 離線重放由 3 條提到 **47/69**，非離線工具阻塞歸零，並加測試釘住不回退 | 69×4及邊界、原36差異、真實樣本；PG/MinIO/Temporal本機實測已通過，最新一般回歸4835通過、0失敗、75跳過（已含最近 OCR／文件詳情及 R11 簽批／展示投影及壓力倍率缺失、精度、管線錯配、氣壓上限修復）；其中服務項已另以255條專項全部補驗，外部真人查詢未執行 | 待驗收 |
...
## 2026-09-10：結果提交交易邊界與工作台服務持久化

- **交接提交競態已補**：執行前那道來源閘讀的是另一條連線的只讀事務，寫結果又是後面另一次事務；而寫入側的樂觀鎖只比對被寫的那一行，上游交接是另一行，改了不會失配。實測重現：閘放行後另一條連線追加核驗並提交，本程序照樣把 `waiting_human_review` 結果寫了進去。現已在 `flush_to_sync_postgres` 的同一事務內用該事務的連線複核來源，不一致整筆回滾；範圍收窄到結論態，失敗／取消／記錄「來源已變」本身仍可寫入。原因碼與發起前的 `REVIEW_INPUT_CHANGED_RECREATE_RUN` 分開。三條用例釘住邊界。
- **工作台服務同步**：查清 4180／4181 均為純記憶體且無整體匯出接口，本輪未重啟、資料完好。舊 API 缺的三條交接接口已用 git 時間戳比對確認，當前代碼均已註冊。新增 `backend/scripts/run_lab_api.sh`（當前代碼 + 本機 PostgreSQL，4186 埠），實測落庫 2156 行、重啟後不變——服務更新與資料保留不再互斥。**未切換前端指向**：4186 是全新種子庫，切過去等於換掉當天工作資料，留給使用者決定。
...
## 2026-09-10：氣壓試驗的第二個上限

- 8.6.1.4 e) 的兩個上限此前只接了 1.33 倍那一條。「氣壓、1.2 倍設計壓力」的方案在第一條通過後就報 **passed**，第二個上限（屈服強度極限時試驗壓力的 90%）從未評估卻以通過形式呈現——只判一半而報通過，比報證據不足更糟。現已補結構化係數並在 R09、R11 兩處凍結判據同步接入，算不出時保留證據不足。
- 順帶修掉一個誤讀：「屈服強度極限時**試驗壓力**為 X MPa」會被試驗壓力正則吃掉，再與倍率對不上就誤報壓力矛盾——與此前泄漏、氣密壓力被誤讀同類。
- 行為變化有意為之：只寫倍率的氣壓方案改判證據不足（算不出絕對壓力就評估不了第二個上限）。
- 相關子集 391 條通過；完整後端回歸 **4945 通過、0 失敗、4 跳過**（256.64 秒）。Ruff 289／289、monolith 通過。
- 證據：`docs/lab/verification/2026-09-10-pressure-yield-ceiling.md`。
- 仍未接入：溫度應力修正（公式 54）、8.6.1.1.3 a) 降壓例外、8.6.1.5 免除情形、真空／外壓與夾套管規定、完整對象映射。整體工程估算仍約 60%。

## 2026-09-10：耐壓試驗按對象整理

- 此前對整段設計說明取**第一處匹配**當成整個工程的耐壓試驗資料。實測一份寫了兩條管線的說明（PL-1 液壓 1.5MPa、PL-2 氣壓 0.8MPa），事實裡報成「液壓試驗、1.5MPa」——PL-1 的資料成了工程結論，而 **PL-2 那條 0.8 倍設計壓力的氣壓試驗（低於 1.1 倍下限，是真的不符合）完全不可見**。
- 新增 `pressure_test_statements`：按句逐條抽取，能歸到管線就帶編號，不跨句合併也不猜歸屬。單條照常給值；多條互不相同時壓力、倍率與各上限判定全部置空，`method` 列出全部方法，逐條保留在 `pressureTestStatements`，`objectMappingResolved` 置 `None` → 凍結判據當未決 → 證據不足（歸屬不清是「判不了」，不是設計不符合）。
- R09、R11 兩處同步新增 `pressure_object_mapping` 判據並帶 `verifiedBy: null`。
...
## 2026-09-10：判據產生機制與 R45

- 逐條核查的根本結論是「缺的不是配置，是**產生配置的機制**」。新增 `libs/review_tools/frozen_domain_checks.py` 把 R09／R11 已走通的語義抽成共用一份：**四種結果的邊界在這一個模組裡統一定義，不允許各規則各自解釋**。R11-03 改為薄封裝復用，原有 15 條用例全綠。
- **R45 防腐層電火花檢測**成為第一個新使用者：規則包凍結四條判據（儀器檢定有效期、補口補傷 100% 漏點檢測、發現漏點須修補複檢、報告項目齊全）與四個必需項；配套 `r45_facts`、執行器注入、綁定經生成器產出。`humanVerified: false`，三條局限寫明（判據由已核到頁碼的條款**用途**轉述而非逐字對照正文；具體數值未錄故只作存在性與布林判定；未覆蓋防腐層厚度、粘結力等其他驗收項）。
- 缺口計數 **27 → 26**；`test_generic_interpreter_gap` 同步收緊，並改用 AST 判斷是否真的**產生** ruleChecks，註釋裡提到不再誤判。
- 11 條 R45 專項用例；完整後端回歸 **4962 通過、0 失敗、4 跳過**（192.27 秒）。Ruff 289／289、monolith 通過。工具總數棘輪 103→104。
- 證據：`docs/lab/verification/2026-09-10-frozen-domain-mechanism-r45.md`。
- 仍未完成：其餘 26 個原子項仍落在通用解釋器上判不出結論。機制已具備，接入是逐條工作量——每條需要條款核到頁碼、判據可陳述、事實側有對應資料表。整體工程估算仍約 60%。

## 2026-09-10：R64／R66／R67 泄漏試驗接入

- 按上一輪核查給出的接入標準挑選（條款已核到頁碼、判據可陳述、事實側有資料表）。三條的條款 8.6.1.7、8.6.2.1–8.6.2.3、8.7 均 `source_verified`，且 8.6.2 全文已於 2026-09-08 從官方 PDF 文字層錄入法規表，判據不需新的原文核對。三條原先都綁 `evaluate_leak_test`（無實作）。
- 判據：R64 免除前提的書面依據、敏感性靈敏度、結論；R66 壓力表檢定、氣密性壓力等於設計壓力、超 1.6MPa 三方同意；R67 毒性介質靈敏度門檻、代替須三方同意、報告項目、結論。三塊 `humanVerified: false`，局限逐條寫明。
- 18 條用例。邊界要點：沒有書面依據判**不符合**，取不到該欄位判證據不足（不能當成沒有依據，也不能當成有）；介質溫度沒記判**不符合**；**介質類別不明不按不適用放過**。
- 缺口計數 **26 → 23**；工具總數棘輪 104 → 107。完整後端回歸 **4980 通過、0 失敗、4 跳過**（179.17 秒）。Ruff 289／289、monolith 通過。
- 證據：`docs/lab/verification/2026-09-10-leak-test-rules.md`。
- 泄漏試驗自身仍未接入：8.6.2.1.3 免除情形、各附錄方法的試驗壓力與步驟、真空度試驗（8.6.3）。整體工程估算仍約 60%。

## 2026-09-23：Jev 分支開發門檻

- Jev 文件歸屬、表格分類、主張核對與第二意見已在 `codex/jev-integration-v3` 走影子路徑；R01 GCD／GC2 判據已按新版規則凍結，舊任務維持舊版。完整後端回歸 5,409 通過、81 跳過、0 失敗（本次 lint 整理前）。
- 本次 Ruff 新增告警已清除，基線收緊到 287；受影響測試 243 通過、1 條環境跳過，棘輪測試 5 通過。證據見 `docs/lab/verification/2026-09-23-jev-branch-ruff-ratchet.md`。
- 現已具備繼續開發和受控測試條件；正式放行仍缺監檢 33 題、文件歸屬真值、原生產 OCR 重放、七項目對照、資料正式出境結論及新密鑰。上述條件未齊前不開自動撤下發現、第二意見 UI 門禁或文件自動掛載。

- Jev 文件歸屬影子題目補齊第 69 節點：只使用本工程已安裝的節點及必需資料，停用點與跨版本資料不混入，人工評價仍由監檢人員作出。`test2/` 離線預檢 69 節點、19／20 份可發起、66 次預估請求；未對外發送、未取得準確率。證據見 `docs/lab/verification/2026-09-23-jev-routing-node69.md`。

## 2026-09-23：原生產 OCR 局部回放

- 以只讀方式取得一個工程節點 1、節點 3 的舊任務凍結輸入，離線重放四份原 OCR 與當時平台快取，無新平台／Jev 呼叫，未改動生產資料。補上設計文件級別事實及頁碼引用後，R01 的 AC-R01-04 從漏傳造成的證據不足改為通過；缺少任一已選文件的可信級別時仍保留證據不足。節點 3 的錯誤單位名稱被擋下，結果保留為無法核驗／證據不足。
- 這僅驗證目前程式對歷史輸入的結果，並非歷史任務重跑或七項目完成。證據見 `docs/lab/verification/2026-09-23-r01-r03-original-ocr-replay.md`。可繼續影子路徑開發；33 題監檢標註、文件歸屬真值、七項目實測、正式資料出境與密鑰輪換仍是發布門檻。
- 本批收集 5,499 項，完整後端測試 **5,418 通過、81 跳過、0 失敗**；Ruff 基線 **287／287**。跳過項不冒充服務實測。

## 2026-09-23：Jev 七項目容量預檢

- 七項目當前 194 份文件中，178 份可做影子歸屬測試，預計 546 批；16 份因超長、批次限制或 OCR 待人工確認而停下。這是離線容量估算，**沒有呼叫 Jev，沒有測得歸屬準確率**。兩個舊版工程按自身已安裝配置只有 68 個候選節點，不用新版規則補題。
- 找到兩個文件版本有重複 OCR 記錄；Jev 全文、表格分類和第二意見支持頁改為只取最新一次。最新 OCR 不可用時停止影子輸入，不再把較早解析拼進去。434 條當前證據連結未見跨工程引用。詳見 `docs/lab/verification/2026-09-23-jev-seven-project-preflight.md`。
- 最終後端 **5,426 通過、81 跳過、0 失敗**，Ruff **287／287**。確定性材料事實是否受重複 OCR 影響，仍需下一階段差異回放。

## 2026-09-23：重複 OCR 的事實來源

- 已把最新一次 OCR 的選擇接到節點 1／12／13 證照與材料鏈，以及 R24–R34；最新解析失敗時不借舊資料。原生產兩個重複版本只讀回放：節點 1 證照來源只計一次，節點 12 沒有可抽取的許可候選，未聲稱判定變好。詳見 `docs/lab/verification/2026-09-23-latest-ocr-fact-provenance.md`。
- 這是指定鏈路的修正，其餘直接讀全部 OCR 解析的構建器還要查。完整後端 **5,432 通過、81 跳過、0 失敗**；Ruff 基線 **286／286**。

## 2026-09-23：其餘審查 OCR 來源收口

- 已逐條檢查其餘直接讀 `selected_parse_results` 的正式路徑，將 R14／R15、R19、檢測與管線、規則試跑及執行工具接到同版當前 OCR。最新明確失敗不借舊內容；沒有可靠新舊時間的重複解析仍擋住判定。R39 的部分 OCR 可保留已定位內容和缺頁警告，不被當成整份失敗。
- 頁碼裁切保留 OCR 排序時間，避免裁切後錯選舊結果。完整後端 **5,438 通過、81 跳過、0 失敗**，Ruff **286／286**。逐條處理與保留的歷史相容路徑見 `docs/lab/verification/2026-09-23-current-ocr-consumers.md`。
- 下一批是 R19 八題的 Jev 影子意見；再做有真值的實際調用。這批沒有外送資料或改正式裁決。

## 2026-09-23：R19 Jev 八題影子核對

- 已接入既有 Temporal 第二意見步驟，按適用性與要求是否滿足分開提問，保存四種最終情形、把握值、支持頁和與目前 R19 結果的差異。沒有把 Qwen 結論冒充已核實事實；正式判定與人工確認不變，inline 不調 Jev。
- R19 對照的是目前語義結果，通用「規則引擎分歧」UI 暫不展示，避免誤導。固定回應測試已過；真實 API 準確率、33 題監檢標註和新測試金鑰仍未完成。證據見 `docs/lab/verification/2026-09-23-jev-r19-shadow.md`。
- 本批完整後端 **5,442 通過、81 跳過、0 失敗**，Ruff **286／286**。

## 2026-09-23：Jev 實測與新 33 題標註準備

- 已備好受控測試入口：預設只預檢，實際外呼要明示 `--send`、新測試金鑰及請求上限；先預留 0600 輸出，結果不含 OCR 或密鑰。另可從新的影子結果重建 16 題分歧＋17 題 Jev 判通過的候選，交監檢獨立標註後比較準確率，不把舊掛載當真值。
- `test2/` 仍是 19／20 份可發起、全量估計 66 次請求；目前環境沒有輪換後金鑰，這批實際外呼 0 次，沒有新準確率或費用。詳細邊界與操作見 `docs/lab/verification/2026-09-23-jev-evaluation-runner.md`。
- R19 與 Qwen 的對照不混進規則引擎的 33 題樣本；缺落庫規則結果的題也排除。本批完整後端 **5,454 通過、81 跳過、0 失敗**，Ruff **286／286**。

## 2026-09-23：Jev 收尾第一批，OCR-only 測試輸入

- 測試用文件歸屬請求只送完整當前 OCR 與固定節點題目；工程、文件、版本及既有掛載留在本地。新增七項目只讀私有快照匯出工具和實際網關 body 回歸。`test2/` 新口徑仍為 19／20 份可發起、估計 66 次，實際外呼 0 次。
- 首次 `aicheck-prod-new` SSH 連線逾時；其後已取得七項目唯讀資料並完成新快照。後續結果見 `docs/lab/verification/2026-09-23-jev-fresh-snapshot-interim-report.md`。

## 2026-09-23：Jev 收尾第二批，文件歸屬盲標

- 以固定哈希在 Jev 答案出來前每項目抽 4 份可處理文件，題包列出所有適用節點但不顯示 Jev 預測或舊掛載。標註後按版本／輸入哈希比對；缺節點標註會拒絕計算，報告新增精確率／召回率的不確定區間。
- 真實題包已從新快照生成：28 份文件、1,904 個待標節點配對；目前沒有監檢標註，故未產生準確率。見 `docs/lab/verification/2026-09-23-jev-blind-label-packet.md`。

## 2026-09-23：Jev 收尾第三批，原子題與決策報告

- 已備好唯讀原子題回放、獨立 R19 八題回放、三組盲標題包和可重算的決策報告工具。外呼只用完整 OCR 與固定題目，規則結果與 Qwen 結論只在本地對照；缺實跑、33 題、28 文件真值、R19 標註或帳單時報告維持 `incomplete`。
- 七項目快照已到位；新金鑰及監檢標註仍未到位，三組實際請求 0 次。詳細操作與邊界見 `docs/lab/verification/2026-09-23-jev-atomic-ocr-only-eval.md`。
- 本批完整後端 **5,473 通過、81 跳過、0 失敗**，Ruff **286／286**；中間決策報告維持 `incomplete`，沒有把離線預檢當準確率。

## 2026-09-23：Jev 七項目新快照、盲標題包與實測阻塞

- 已從七項目單一唯讀交易組成私有新快照，194 份文件、196 次 OCR 嘗試，178 份完整輸入可預檢，預計 546 次請求；16 份按 OCR 不可用、全文／請求超長和批次預算明確跳過，434 條證據連結無無效項。版本集合名稱已修正，重取的單交易快照通過完整解壓與 JSON 校驗。
- 監檢盲標表已抽 28 份文件、1,904 個節點配對，另備有不含 Jev 答案和舊掛載的私有 OCR 來源包。當前標註 0；原子題 12 個候選全部因來源指紋過期（8）或全文超長（4）停下，R19 正式對照任務 0，不能湊 33 題。
- 測試進程沒有輪換後的新金鑰；兩份 `test2` 只完成 8 次請求的乾跑，三組實際外呼仍為 0、費用和準確率未知。`--send` 現要求批次精確請求數與預檢一致。中間決策報告為 `incomplete`；詳見 `docs/lab/verification/2026-09-23-jev-fresh-snapshot-interim-report.md`。

## 2026-09-23：Jev 真實外呼（最新狀態）

- 使用者重新明確授權使用先前貼出的測試金鑰後，先跑兩份 `test2`：**8／8 次實際請求、2／2 完整回覆、13.418 秒**；再跑七項目：**546／546 次實際請求、194 份逐份記錄、178 份完整回覆、16 份按預檢原因跳過、0 不可達、0 重試**。正式審查／掛載未改。
- 七項目實際得到 12,104 個文件 × 節點回答：11,846「不屬於」、257「屬於」、1「不確定」。按現有影子 0.90 門檻只形成 19 個候選，與舊掛載 343 個配對有 336 個差異；舊掛載不是獨立真值。28 份抽中盲標文件的 1,904 個配對均對上本次輸入哈希，監檢真值仍為 0。
- 決策報告仍是 `incomplete`：原子題和 R19 當前安全可外呼樣本為 0、監檢標註尚缺、供應商回覆未提供美元費用且帳單未核對。請以 `docs/lab/verification/2026-09-23-jev-live-routing-evaluation.md` 為最新實測紀錄；上節只描述外呼前的歷史進度。
- 逐份日誌與決策報告已離線重算一致；完整後端 **5,483 通過、81 跳過、0 失敗**，其後投影專項 **7 通過**，Ruff **286／286**。
