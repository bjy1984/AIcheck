...
| 交接 | 草稿、來源檢查、核驗API；原工作台建立入口與授權接收任務查詢；PostgreSQL跨連線核驗衝突實測通過；新任務交接輸入／依賴失效與工作程序按需載入、前端選用／需重驗提示已接線；正常登入選用與新任務切換已驗 | 登入建立交接／真實原文／視覺與鍵盤驗收、執行中途來源變動及HTTP多程序併發驗收 | 待接線／驗收 |
