...
| R35–R37、R39 | 專用核心、凍結事實及實際綁定；R39指定文件對及有來源的聲明文件清單引用覆蓋、異常文件對及逐對來源可信度隔離已接線並合成驗證；PT乳化劑施加方法、完整文件內容、應用事件、簽核週期、跨清單版本對照及尚未應用有據排除已接線及合成驗證 | R39專用OCR入口已補；單對原文欄位到引用版次比對已接通；機構身份核實、其他業務事實及多文件適配待補，標準化表測試不代表真實解析已完成；剩餘技術能力與完整規則驗收，三類pendingCapabilities仍阻擋發布 | 待補齊 |
