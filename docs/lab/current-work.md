...
| 結果UI | 分層卡片、口語提示、完整清單、證據定位；已接入待辦首頁；空结果与执行状态分开，排队实机已验；系统能力未完成独立提示已接线；R11 簽批逐項口語清單已接原首頁並完成合成瀏覽器／實際 API 回傳驗收，共用原文定位已補版本及頁碼矛盾阻擋 | 已登入種子工程選資料／人工保存已驗；仍需帶真實原文的完整案件及可用性驗收 | 待驗收 |
