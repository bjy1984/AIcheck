...
- 每份文件以本工程、当前版本的完整 OCR 为输入，按当前业务包启用的审查点汇总成
  ...
  `test2-019` 的 30,691 字 OCR 需分成 6 次完整原文请求，`test2-020` 的 3,163 字需
  3 次请求。这里仅计算请求尺寸，未实际发送这批文件归属题，也未测准确率与费用。
...
- 文件归属的多批请求可能长于普通任务；只给这个任务的 PostgreSQL 锁设 150 秒空闲
  超时，普通任务仍为 60 秒，避免请求未结束时锁先失效，同时保留孤儿锁自动清理。
- 这一批用伪造 Jev 响应验证了整份 OCR、人工否决、输入变化重跑、异步派发、版本
  竞态及故障回退；统一客户端和任务锁加固后相关后端回归 **104 项通过**。**尚未用七项目真值集衡量归属准确率**，不得把影子建议接入自动
