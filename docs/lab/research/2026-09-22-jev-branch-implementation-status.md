...
1. **资料出境合规决定与密钥轮换**。现有密钥曾在聊天出现，不能复用；新密钥由运维
   放入 `/home/dev-bjy/aicheck-secrets.env`。没有明确决定时保持所有 Jev 开关关闭。
...
- Jev 单元测试、条件清单一致性和 P8/H5 清单回归：56 项通过（2026-09-22）。
...
- 测试全部使用伪造/录制响应；未用真实密钥、未向 Jev 发送工程资料，也未在生产部署。
