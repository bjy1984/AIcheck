...
- 后端全量：2359 passed，33 skipped；
- 前端：70 个单测通过，TypeScript 检查通过，ESLint 0 error，测试构建成功；
- 浏览器：确认入口并列、空工程阻断、已有资料工程 Preview 计数、运行恢复和真实准备进度；控制台 0 error；
- 工程分析契约、OpenAPI 和部署检查通过；
- 模型阶段、完成态和失败态展示由纯展示逻辑单测覆盖；实际 LLM 调用由部署环境执行。
