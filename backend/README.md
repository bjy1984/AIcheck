...
- `embedding-service`: local Infinity embedding server for `BAAI/bge-m3`. It serves `embedding-default` through LiteLLM, keeps model files in `AICHECK_EMBEDDING_CACHE_HOST_PATH`, and can use `HF_ENDPOINT` for first-run model downloads on restricted networks.
...
Add `--litellm-provider-probes` when you want a quota-consuming production check that calls `default-chat` and `embedding-default` through LiteLLM. `default-chat`, `review-chat`, and `compare-fast` route to DeepSeek `deepseek-reasoner`; `embedding-default` routes to the local Infinity `BAAI/bge-m3` service and should return 1024-dimensional vectors.
...
- `INFINITY_API_KEY`, `AICHECK_EMBEDDING_MODEL_ID=BAAI/bge-m3`, `AICHECK_EMBEDDING_ENGINE=torch`, `AICHECK_EMBEDDING_CACHE_HOST_PATH`, `HF_ENDPOINT`: local embedding-service configuration. First startup downloads the model into the cache path; after that, restarts reuse the local cache. `HF_ENDPOINT=https://hf-mirror.com` is useful on China-hosted servers where direct Hugging Face access is unreliable. BGE-M3 vectors are 1024-dimensional, so vector metadata and FDE mock data should use dimension 1024.
