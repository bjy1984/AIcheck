...
def create_review_run_from_ai_run(ai_run: dict[str, Any], *, mode: str = "temporal") -> dict[str, Any]:
    ...
    record = {
        ...
        **review_run_evidence_lineage(ai_run),
...
def _execute_review_run_inline(review_run_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        if isinstance(exc, shard_execution.EvidenceShardProcessingIncomplete):
            return shard_execution.mark_review_run_incomplete(review_run, ai_run, exc, bump_revision=bump_review_run_revision, append_event=append_review_event)
...
def _generate_finding_drafts_once(
    ...
    logical_call_id = f"review:{review_run['reviewRunId']}:generate_findings{f':{evidence_shard_id}' if (evidence_shard_id := str(context.get('evidenceShardId') or '')) else ''}"
    previous_attempts = [
        item
