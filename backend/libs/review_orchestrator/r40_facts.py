...
def build_r40_business_facts(state, run):
    ...
    issues = []
    latest = current_selected_parse_results(state, {}, context={"reviewRun": run}, include_unusable=True)
    parses = [row for row in latest if ocr_parse_usable(row)]
    for version_id in sorted(set(run.get("inputDocumentVersionIds") or [])):
        ...
        candidates = [row for row in latest if row.get("documentVersionId") == version_id
                      and row.get("tenantId") == run["tenantId"]]
        if (len(versions) != 1 or len(documents) != 1 or len(candidates) != 1
                or not ocr_parse_usable(candidates[0])):
