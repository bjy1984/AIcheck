...
def _identity(run: dict[str, Any], *, include_outputs: bool = True) -> dict[str, Any]:
    ...
    version = {"identity": values, "station": station, "documents": run.get("documentScopeSnapshot"),
               "documentVersionIds": run.get("inputDocumentVersionIds"), "rule": run.get("effectiveRuleSnapshot")}
    if include_outputs:
        version.update(status=run.get("status"), outputHash=run.get("outputHash"), findings=run.get("findingDrafts"))
    values["versionHash"] = digest(version)
    return values
...
def create_handoff_draft(source: dict[str, Any], target: dict[str, Any], *, kind: str,
    ...
    return _build_handoff_draft(source, target, kind=kind, subject=subject, payload=payload,
                                evidence_refs=evidence_refs, schema="review-handoff-draft-v3")


def _build_handoff_draft(source, target, *, kind, subject, payload, evidence_refs, schema):
    """Reconstruct older identities for historical validation; no public downgrade path."""
    ...
    if schema in {"review-handoff-draft-v2", "review-handoff-draft-v3"}:
        fields.add("eventId")
    ...
    if schema in {"review-handoff-draft-v2", "review-handoff-draft-v3"} and (
    ...
    origin, recipient = _identity(source), _identity(target, include_outputs=schema != "review-handoff-draft-v3")
...
def validate_handoff_draft(record: dict[str, Any], source: dict[str, Any], target: dict[str, Any],
    ...
    if record.get("schemaVersion") not in {"review-handoff-draft-v1", "review-handoff-draft-v2", "review-handoff-draft-v3"}:
    ...
    if record.get("source") != _identity(source) or record.get("target") != _identity(target, include_outputs=record["schemaVersion"] != "review-handoff-draft-v3"):
