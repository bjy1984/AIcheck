    ...
    *,
    ...
    if tool_name == "evaluate_r37_progressive_inspection" and "event" not in arguments:
        for key in ("projectId", "organizationId", "progressiveInventory", "progressiveEvents", "inspectionBatches", "inspectionBatchMembers", "progressiveReports"):
            arguments.setdefault(key, deepcopy(nested_dict(facts, "r37").get(key)))
    if tool_name == "evaluate_r37_reinspection" and "case" not in arguments:
        for key in ("projectId", "organizationId", "applicability", "caseInventory", "originalInspections", "dispositions", "reinspections"):
            arguments.setdefault(key, deepcopy(nested_dict(facts, "r37").get(key)))
    if tool_name == "evaluate_ndt_nonconformance":
        for key in ("projectId", "organizationId", "applicability", "procedure", "caseInventory", "commissions", "notices", "feedback"):
            arguments.setdefault(key, deepcopy(nested_dict(facts, "r37").get(key)))
    r39_inputs = {"evaluate_r39_first_use_validation": "firstUseValidation", "evaluate_r39_approval_chain": "approvalChain", "evaluate_r39_document_content": "documentContent"}
