...
from libs.review_tools.r39_inventory_consistency import evaluate_r39_inventory_consistency
...
BUSINESS_TOOL_CAPABILITIES = {
    ...
    "evaluate_r10_standard_adoption": "按冻结判据核对是否采用了其他标准：设计说明书是否载明设计依据与遵循的标准规范、结论是否与 TSG 31-2025 1.9 条一致、站场工艺管道是否用了准许的标准；只判识别这一步，申明与比照表由另两项判定。",
    "evaluate_r10_compliance_declaration": "按冻结判据核对符合性申明与比照表是否存在、是否载于设计文件或工程规定之中；只判有没有，申明内容是否属实需人工核验。",
    "evaluate_r10_comparison_table": "按冻结判据核对比照表覆盖：是否覆盖 TSG 31-2025 附件B 的 15 个行目、每行是否有符合性申明与两栏填写、不符合项是否按 1.9 条办理；每行的技术结论是否成立不在判定范围。",
    "evaluate_r56_accessory_documents": "按冻结判据核对安全附件证明文件：是否经型式试验合格且未超出证书覆盖范围、产品标准是否在附件B内否则是否提供比照表、出厂三类资料是否齐全、质量证明书是否由质量保证工程师签署并注明生产许可证编号、铭牌是否含许可证编号、材料质量证明书是否可追溯。",
    "evaluate_r56_accessory_installation": "按冻结判据核对安全附件现场选用与安装：规格型号是否符合设计、总泄放量是否满足系统、进出口流道面积是否不小于端口、安全阀是否铅直安装且入口压力损失不超过整定压力 3%、爆破片是否无损伤同轴且铭牌可见、紧急切断阀切断型式与安装方位是否正确。",
    "evaluate_r57_safety_valve_calibration": "按冻结判据核对安全阀校验报告：校验周期是否不超过 5 年、校验介质是否与使用介质对应、整定压力是否做满 3 次重复性试验且均合格、密封试验是否合格、在线校验是否有正当理由、报告项目是否齐全并签署。",
    "evaluate_r58_emergency_valve_test": "按冻结判据核对紧急切断阀性能测试：出厂检验项目是否齐全、多功能阀是否逐项试验、质量证明书与铭牌项目是否完整并签章、使用期间定期自行检查是否进行、异常情况是否停用修理或更换。",
    "evaluate_r43_material_certificate": "按冻结判据核对检查合格证和记录：是否审阅合格证、质量证明书与标记、材料与组成件是否为规定等级、是否已经过要求的热处理检查和试验、是否向检验人员提交质量控制执行的证明文件；证书真伪与签署权限属人工核验。",
    "evaluate_r44_coating_construction": "按冻结判据核对防腐、补口、补伤：组对前预制防腐层是否检查、补口补伤是否 100% 漏点检测且无漏点、厚度是否四方位测点并满足设计、粘结力是否检测评定、回填后是否全线破损点定位检测、定向钻穿越段电阻率是否不小于 5000 Ω·m²、质量证明文件是否完整且可追溯。",
    "evaluate_r46_cathodic_protection": "按冻结判据核对阴极保护施工与验收：绝缘装置两侧电阻是否大于 10 MΩ 且测试条件符合、牺牲阳极填包料是否完全包覆、强制电流电源额定参数是否符合设计、通电前电缆是否已正确连接、交接验收六类技术文件是否齐全、安装记录项目是否完整。",
    "evaluate_r47_static_grounding": "按冻结判据核对静电接地：接头跨接电阻是否不大于 0.03Ω、对地电阻是否不大于 100Ω（山区等高土壤电阻率场所 1000Ω）、不锈钢与有色金属管道是否用同材质连接板过渡、安装后是否测试并在超限时调整。",
    "evaluate_r48_weld_layout": "按冻结判据核对焊缝设置：环焊缝间距是否满足壁厚倍数与 DN 门槛下限、距弯管起弯点与支吊架、开孔边缘的净距、焊缝上开孔与十字焊缝是否经无损检测合格、穿越处是否设套管且套管内环焊缝是否 100% 检测。",
    "evaluate_r49_crossing_construction": "按冻结判据核对穿跨越工程施工：穿越道路墙体楼板时是否加设套管或涵洞、套管内是否有焊缝、穿墙套管长度是否覆盖墙厚、穿楼板套管是否高出楼面 50 mm、管道与套管间是否填塞不燃材料、埋地段防腐与回填时序是否正确并留有隐蔽工程记录。",
    "evaluate_r50_sleeve_insulation": "按冻结判据核对套管防腐绝缘：防腐等级是否按土壤腐蚀性确定、穿越道路铁路及变深弯管处是否为特加强级、直埋段是否与地上管道绝缘隔断、钢套管间是否设绝缘支撑且两端以非导电材料密封。",
    "evaluate_r53_installation_connections": "按冻结判据核对布管与连接：是否强力对接、法兰平行度是否不大于 1 mm/200 mm、螺栓孔偏移是否不大于 3 mm、每对法兰是否只用 1 个垫片、GC1 法兰接头是否有经批准的书面程序、螺纹与卡套等其他连接的装配要求。",
    "evaluate_r52_prefabrication": "按冻结判据核对管道现场制作：热切割后是否清除熔渣、标记是否保存或移植、低温钢与不锈钢是否误用硬印、弯管不圆度是否不大于 8%（外压 3%）、褶皱高度与波浪间距、板焊管筒节长度与纵缝错开、棱角度与直度允差、斜接弯头是否全焊透。",
    "evaluate_r53_equipment_connection": "按冻结判据核对管道与设备的连接：是否在设备定位并紧固地脚螺栓后连接、动设备接口的平行度同心度是否在表 41 允许偏差内、最终连接时位移值是否小于 0.02／0.05 mm、试压吹扫合格后是否做复位检查。",
    "evaluate_r54_compensator": "按冻结判据核对补偿装置安装：导流标志是否与介质流向一致、是否用膨胀节变形弥补安装偏差、吊装是否作用在波纹管上、导向与固定支架安装完毕前是否做了压力试验、运输固定装置是否在正确时机拆除。",
    "evaluate_r55_supports": "按冻结判据核对支吊架：制作是否符合设计与产品文件、全焊透焊缝抽检是否不少于 20% 且长度不小于 200 mm、吊杆是否垂直、固定支架是否在预拉伸前固定、无补偿直管段是否只设一个固定支架、弹簧临时固定件拆除时机、滑动面是否卡涩。",
    ...
    "evaluate_r39_first_use_validation": "按指导书版本、对象及事件核对首次应用验证；具来源的完整应用清单逐项执行并列出漏查事件，不代替验证有效性、技术符合性或整条R39。",
...
BUSINESS_TOOL_DESCRIPTORS: list[dict[str, Any]] = [
    {
        ...
        "inputSchema": (
            ...
            if name == "check_license_registry_match"
                  ...
                  "ownerApproval": "object?", "planUsage": "object?", "planUsages": ["object?"], "usageInventory": "object?", "selectionIssues": ["object?"], "facts": "object?", "requiredFields": ["string?"], "ruleChecks": ["object?"]}
            ...
            else {"projectId": "string", "referenceInventory": "object", "documentInventory": "object", "approvalInventory": "object", "applicationInventory": "object", "applicationDocumentLinks": ["object"], "unappliedInstructions": ["object?"]}
            if name == "evaluate_r39_inventory_consistency"
...
def dispatch_business_tool(tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    handlers: dict[str, Callable[[dict[str, Any]], dict[str, Any]]] = {
        ...
        "evaluate_r46_cathodic_protection": evaluate_r46_cathodic_protection,
        "evaluate_r47_static_grounding": evaluate_r47_static_grounding,
        "evaluate_r48_weld_layout": evaluate_r48_weld_layout,
        "evaluate_r10_standard_adoption": evaluate_r10_standard_adoption,
        "evaluate_r10_compliance_declaration": evaluate_r10_compliance_declaration,
        "evaluate_r10_comparison_table": evaluate_r10_comparison_table,
        "evaluate_r56_accessory_documents": evaluate_r56_accessory_documents,
        "evaluate_r56_accessory_installation": evaluate_r56_accessory_installation,
        "evaluate_r57_safety_valve_calibration": evaluate_r57_safety_valve_calibration,
        "evaluate_r58_emergency_valve_test": evaluate_r58_emergency_valve_test,
        "evaluate_r43_material_certificate": evaluate_r43_material_certificate,
        "evaluate_r44_coating_construction": evaluate_r44_coating_construction,
        "evaluate_r49_crossing_construction": evaluate_r49_crossing_construction,
        "evaluate_r50_sleeve_insulation": evaluate_r50_sleeve_insulation,
        "evaluate_r52_prefabrication": evaluate_r52_prefabrication,
        "evaluate_r53_installation_connections": evaluate_r53_installation_connections,
        "evaluate_r53_equipment_connection": evaluate_r53_equipment_connection,
        "evaluate_r54_compensator": evaluate_r54_compensator,
        "evaluate_r55_supports": evaluate_r55_supports,
        ...
        "evaluate_r39_inventory_consistency": evaluate_r39_inventory_consistency,
