"""Reconcile a sourced inventory of document approval cycles before aggregation."""
...
def evaluate_approval_inventory(arguments, fields, evaluate_one):
    inventory, approvalCycles = arguments.get("inventory"), arguments.get("approvalCycles")
    ...
    def finish(status, reason):
        output = result("evaluate_r39_approval_chain", status,
            facts={"scope": "declared_complete_approval_cycle_inventory", "reason": reason,
                   "cycleResults": outputs, "technicalCompliance": "not_evaluated",
                   ...
                   "coverage": {"inventoryValidated": valid, "requiredCount": len(required) if valid else None,
                                ...
                                "missingCycles": [dict(zip(fields, key, strict=True)) for key in sorted(required - seen)] if valid else []}},
            checks=[], rule_version="r39-approval-cycle-inventory-v1")
    ...
    if (not isinstance(inventory, dict) or inventory.get("projectId") != arguments.get("projectId")
        ...
        return finish("evidence_insufficient", "r39_complete_approval_cycle_inventory_missing")
    members = inventory.get("members")
    if not isinstance(members, list) or not members or not isinstance(approvalCycles, list):
        return finish("evidence_insufficient", "r39_approval_cycle_inventory_members_missing")
    for member in members:
        if (not isinstance(member, dict) or member.get("projectId") != arguments.get("projectId")
            ...
            return finish("evidence_insufficient", "r39_approval_cycle_inventory_member_invalid")
        key = tuple(member[field] for field in fields)
        if key in required:
            return finish("evidence_insufficient", "r39_approval_cycle_inventory_member_duplicate")
        required.add(key)
        member_refs.extend(_refs(member))
    valid = True
    candidates = {}
    for document in approvalCycles:
        if not isinstance(document, dict) or {"inventory", "approvalCycles"} & document.keys():
            invalid = True
            continue
    ...
    for key, matches in sorted(candidates.items()):
        ...
        output["cycleScope"] = deepcopy(matches[0]["scope"])
        outputs.append(output)
    statuses = {row["result"] for row in outputs}
    if "failed" in statuses:
        return finish("failed", "r39_known_approval_failure")
    if invalid:
        return finish("evidence_insufficient", "r39_approval_cycle_invalid_or_duplicate")
    if required != seen or "evidence_insufficient" in statuses:
        return finish("evidence_insufficient", "r39_approval_cycle_inventory_incomplete")
    return finish("not_applicable" if statuses == {"not_applicable"} else "passed", "r39_approval_cycle_inventory_compared")
