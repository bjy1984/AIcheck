"""Reconcile sourced instruction/procedure pairs before aggregating reference results."""
...
def evaluate_reference_inventory(arguments, fields, evaluate_one):
    inventory = arguments.get("inventory")
    pairs = arguments.get("referencePairs")
    ...
    def finish(status, reason):
        output = result("evaluate_r39_procedure_reference", status,
            facts={"scope": "declared_complete_reference_pair_inventory", "reason": reason,
                   "pairResults": outputs, "technicalCompliance": "not_evaluated",
                   ...
                   "coverage": {"inventoryValidated": valid, "requiredCount": len(required) if valid else None,
                                ...
                                "missingPairs": [dict(zip(fields, key, strict=True)) for key in sorted(required - seen)] if valid else []}},
            checks=[], rule_version="r39-reference-inventory-v1")
    ...
    if (not isinstance(inventory, dict) or inventory.get("projectId") != arguments.get("projectId")
        ...
        return finish("evidence_insufficient", "r39_complete_reference_inventory_missing")
    members = inventory.get("members")
    if not isinstance(members, list) or not members or not isinstance(pairs, list):
        return finish("evidence_insufficient", "r39_reference_inventory_members_missing")
    for member in members:
        if (not isinstance(member, dict) or member.get("projectId") != arguments.get("projectId")
            ...
            return finish("evidence_insufficient", "r39_reference_inventory_member_invalid")
        key = tuple(member[field] for field in fields)
        if key in required:
            return finish("evidence_insufficient", "r39_reference_inventory_member_duplicate")
        required.add(key)
        member_refs.extend(_refs(member))
    valid = True
    candidates = {}
    for pair in pairs:
        if not isinstance(pair, dict) or {"inventory", "referencePairs"} & pair.keys():
            invalid = True
            continue
        ...
        if key not in required or pair.get("projectId") != arguments.get("projectId"):
            invalid = True
            continue
    ...
    if "failed" in statuses:
        return finish("failed", "r39_known_reference_difference")
    if invalid:
        return finish("evidence_insufficient", "r39_reference_pair_invalid_or_duplicate")
    if required != seen or "evidence_insufficient" in statuses:
        return finish("evidence_insufficient", "r39_reference_inventory_incomplete")
    return finish("not_applicable" if statuses == {"not_applicable"} else "passed", "r39_reference_inventory_compared")
