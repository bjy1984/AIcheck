...
def validate_conditions(value: Any) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != {"schemaVersion", "checks"} or value.get("schemaVersion") != "rule-conditions-v1":
...
def evaluate_conditions(conditions: dict[str, Any], facts: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"result": "fail" if "fail" in statuses else "evidence_insufficient" if "evidence_insufficient" in statuses else "pass", "checks": results}
