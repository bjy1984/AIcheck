...
# 月、日后面不许紧跟数字：「2024.11-2028.10」里的首个日期只能是「2024.11」，不能吞成「2024.11-20」。
ANY_DATE_TEXT = r"(?:19|20)\d{2}\s*[.。,\-/年]\s*\d{1,2}(?!\d)(?:\s*[.。,\-/月]\s*\d{1,2}(?!\d)\s*日?|\s*月)?"
ANY_DATE_RE = re.compile(ANY_DATE_TEXT)
# 区间分隔符含连字符/短横（名单表常写「2024.11-2028.10」「2024.11–2028.10」）；两端都须是带年份的日期。
VALIDITY_RANGE_RE = re.compile(
    r"自?\s*(" + ANY_DATE_TEXT + r")\s*(?:至|到|—|–|－|-|~|～)\s*(" + ANY_DATE_TEXT + r")"
)
