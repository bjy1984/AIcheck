...
def resolve_review_input_selection(services, request, project_id: str, node_id: int,
    ...
    visible = {str(row.get("id") or "") for row in detached.state.get("versions", [])}
    visible.update(str(row.get("currentVersionId") or "") for row in detached.state.get("documents", []))
    if set(versions) - visible:
