...
# → 补 AC-R49-01（穿跨越施工，GB 50235-2010 扫描件经 MinerU OCR 重建后核对）得 12
# → 补 AC-R43-01（检查合格证）／AC-R44-01（防腐补口补伤，正文取自知识库已入库条款）得 10
# → 补 AC-R10-01／02／03（采用其他标准的符合性申明与比照表）得 7 → 补 AC-R46-01（阴极保护施工与验收）得 6 → 补 AC-R57-01（安全阀校验报告）／AC-R58-01（紧急切断阀性能测试）得 4 → 补 AC-R56-01／02（安全附件证明文件与现场安装）得 2。
# R45 是第一条走通"判据冻结进规则包 + frozen_domain_checks 统一判定"这条路的通用解释器规则。
EXPECTED_UNCONFIGURED = 2
EXPECTED_RULES = {
    "R11", "R51",
}
