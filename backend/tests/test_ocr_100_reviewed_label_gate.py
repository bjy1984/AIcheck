...
def test_reviewed_label_gate_blocks_unreviewed_tasks(tmp_path: Path) -> None:
    ...
    assert {failure["code"] for failure in report["failures"]} == {"ANNOTATION_READINESS_NOT_READY"}
...
def test_reviewed_label_gate_imports_label_studio_export_before_readiness(tmp_path: Path) -> None:
    ...
    report = run_reviewed_label_gate(tasks, label_studio_export=label_export, output_dir=tmp_path / "gate")
