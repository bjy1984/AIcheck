...
OFFLINE_TOOLS = {
    ...
    "check_required",
    # R11／R13~R23／R25~R34 的工具。2026-09-10 掐掉 socket 跑它们各自的测试套件
    # （96 条，含 r11/r13/r14/r15/r16~r18/r20~r23/r24~r34）确认执行期间不连网。
    # 用运行时验证而不是导入闭包：每个工具都经 runtime_tools 够得着外部登记查询的
    # 客户端，够得着和会去调是两回事，按闭包判会把全部工具都判成不可离线。
    # 唯一真的连网的是 verify_welder_on_platform（R24 的 AC-R24-01），
    # 它调 search_cnse_persons_tool 去查公示平台，故不在此列，R24 也不进登记本。
