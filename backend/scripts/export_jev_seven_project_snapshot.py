...
def build_snapshot(state: dict[str, Any], project_ids: tuple[str, ...] = PROJECT_IDS) -> dict[str, Any]:
    ...
    projects = [{key: row[key] for key in ("id", "businessPackId", "businessPackVersion",
                                          "businessPackSnapshot") if key in row}
                for row in state.get("projects") or [] if row.get("id") in project_ids]
    ...
    documents = [{key: row[key] for key in ("id", "projectId", "currentVersionId",
                                           "tenantId", "fileName") if key in row}
                 for row in state.get("documents") or [] if row.get("projectId") in project_ids]
    ...
    links = [{key: row[key] for key in ("projectId", "documentVersionId", "nodeId", "manualStatus")
              if key in row} for row in state.get("node_evidence_links") or []
             if row.get("projectId") in project_ids
             and str(row.get("documentVersionId") or "") in current_ids]
    runs = [{key: row[key] for key in ("reviewRunId", "id", "projectId", "tenantId", "nodeId",
                                         "reviewMode", "advisoryOnly", "inputDocumentVersionIds",
                                         "inputDocumentPageRanges", "documentScopeSnapshot",
                                         "businessPackId", "effectiveRuleSnapshot",
                                         "atomicCheckToolBindingsSnapshot") if key in row}
    ...
    snapshot = {"source": "read_only_seven_project_ocr_snapshot",
                ...
                "review_runs": runs,
                "rule_check_results": [{"reviewRunId": row.get("reviewRunId"),
                                        "atomicCheckResults": [{"atomicCheckId": item.get("atomicCheckId"),
                                                                "result": item.get("result")}
                                                               for item in row.get("atomicCheckResults") or []
                                                               if isinstance(item, dict)]}
                                       for row in state.get("rule_check_results") or []
                                       if str(row.get("reviewRunId") or "") in run_ids],
                # The frozen source fingerprint hashes each complete row. Keep
                # these local-only sources intact; none enters a Jev request.
                "fact_corrections": [row for row in state.get("fact_corrections") or []
                                     if row.get("projectId") in project_ids],
                "extracted_fields": [row for row in state.get("extracted_fields") or []
                                     if str(row.get("documentVersionId") or "") in current_ids],
                "evidence_links": [row for row in state.get("evidence_links") or []
                                   if str(row.get("documentVersionId") or "") in current_ids],
    ...
    try:
        with psycopg.connect(database_url) as connection:
            ...
            return {"admin_config": row[0], "projects": projects, "documents": documents,
                    "versions": rows("document_versions", "AND object_id = ANY(%s)", current_ids),
                    ...
                    "extracted_fields": rows("extracted_fields",
                                             "AND payload->>'documentVersionId' = ANY(%s)", current_ids),
...
def main() -> int:
    ...
    try:
        if args.state or args.state_zlib:
            source = args.state or args.state_zlib
