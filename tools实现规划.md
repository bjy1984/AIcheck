...
### 1.1 2026-07-17 R24-R34 专项实施结果

R24-R34 已由“部分示例解析或通用规则解释器”升级为专用确定性审核链：

- 当前业务包含 194 条 atomicCheck 绑定、96 个唯一 Tool；109 条为 `pilot_implemented`，85 条为 `implemented`。
- 新增 `r24_r34_facts.py`，从焊工资格证、WPS/PQR、管线汇总表、焊材证明和管理记录、组对/施焊/外观/返修记录、热处理工艺/仪表/曲线/硬度报告装配 R24-R34 事实。
- 新增并接入 WPS/PQR 覆盖、焊材质量证明、焊材管理、组对、施焊、外观、返修、PWHT 适用性、热处理工艺/结果和测温仪表专用 Tool；缺少关键事实、产品牌号规则或设计限值时一律返回 `evidence_insufficient`。
- R24 按 TSG Z6002-2010 解析方法、材料、位置、厚度、直径和工艺因素；2026-08-01 后在 TSG Z6002-2026 规则包未核定前 fail-closed。
- R32、R33 与 R34 共用焊口级 PWHT 适用性键；R34 硬度限值按材料/设计条件判断，不再使用统一 200HB/225HB。
- OCR 新增并路由管线汇总、焊材质量证明/管理、组对、外观、返修、热处理工艺/仪表/曲线和硬度报告 profile。

当前专项实现仍标记为试点：现场人证相符、实物批号、实物组对/外观等必须由监检人员上传现场见证；粘接工艺需另配相应非金属标准；焊材化学和力学性能只有在对应牌号产品标准 profile 已冻结时才能自动判定。

### 1.2 2026-07-12 第一轮实施结果（历史基线）
...
1. 将 194 条 atomicCheck 绑定转换为可执行、可重复、可测试的 Tool 执行计划。
