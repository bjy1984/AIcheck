...
- 模型供应商密钥：AI 审查主链路使用 `DEEPSEEK_API_KEY`，默认模型为 DeepSeek `deepseek-reasoner`；默认 `embedding-default` 指向项目内置的本地 embedding-service，底层默认 `Qwen/Qwen3-Embedding-0.6B`，`BAAI/bge-m3` 保留为 fallback，不依赖 `OPENAI_API_KEY`。
- 可访问的域名和 HTTPS 证书。
...
| `embedding-service` | 7997 | `127.0.0.1:7997` | 本地 OpenAI-compatible embedding API；默认 Qwen3-Embedding-0.6B，只供 LiteLLM 内网调用 |
...
| `INFINITY_API_KEY` | 是 | 本地 embedding 服务 API key。变量名保留为兼容 LiteLLM provider 配置；只通过环境变量注入，不应出现在 `docker ps` 命令行参数中。 |
...
本地 `embedding-service` 默认懒加载模型，`/health` 可先健康启动。真实执行 Qwen3-Embedding-0.6B 向量化时建议为 Docker 分配至少 4-6 GB 内存；2 GB Docker VM 可以启动服务，但首次加载模型可能失败。
...
默认 `embedding-default` 已对齐到本地 embedding-service 的 served-model alias，底层默认 `Qwen/Qwen3-Embedding-0.6B`。开启 `AICHECK_LITELLM_STRICT_PROVIDER_HEALTH=true` 时要求 `embedding-service`、`INFINITY_API_KEY` 和 `backend/config/litellm.yaml` 一致，不要求 `OPENAI_API_KEY`。只有自行新增 OpenAI-backed alias 时才需要提供 `OPENAI_API_KEY`。
