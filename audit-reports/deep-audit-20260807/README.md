...
## 全量修复轮（2026-08-11 第二批）

在业务方决策落地后，把剩余发现按危害逐批修完并在运行中的服务上实测。

### 已修

| 项 | 内容 | 提交 |
|---|---|---|
| M-3 | 上传会话响应回显完整 JWT → 只回 Content-Type 与短时上传凭证；前端改为自带登录态 | `2f38b43` |
| M-9 | 建设方（observer）可读 AI 判定理由 / 监检人工结论 / 整改往返 → 三个独立端点 + 节点包同口径裁剪 | `2f38b43` |
| M-11 | 建设方看到施工方尚未提交的草稿 → 与监检同一口径（已提交才进入视野） | `2f38b43` |
| M-8 | 上传声明的 materialTypeCode / nodeIds 静默丢弃 → 按业务包解析名称、声明的节点真正落库 | `579b07d` |
| — | **新发现**：`POST /documents/bindings` 忽略逐条声明的 nodeId，全部落到默认节点 16 → 按声明分派 | `579b07d` |
| M-7/M-10 | 说明文字传错字段名即静默丢弃 → 明确拒绝并说清该用哪个字段 | `579b07d` |
| M-6 | 打回后无法用新上传的资料补正 → 放行本节点新挂载资料，记录 replacementBindingIds | `579b07d` |
| M-4 | 空壳记录在台账里无可视区分 → 每行带 bodyUploaded | `579b07d` |
| M-5 | 重复上传无去重提示 → 按 hash + projectId 提示（不阻断） | `579b07d` |
| M-1 | 分类硬编码 stub（准确率 21%）→ 按业务包 nodeTemplates.requiredMaterials 分类，**88%** | `6f35052` |
| N-3 | 置信度四处硬编码 → 由证据锚定质量派生，并带 confidenceBasis | `0d06283` |
| N-4 | 节点状态机无转移校验 → 拦住「已办结 → 流程起点」 | `0d06283` |
| N-5 | 多租户重启后非 configured 租户数据永不加载 → 加载按租户参数化 | `0d06283` |
| N-6 | If-Match 缺省即放行 → 四类高风险写强制要求（默认开启 + 迁移开关） | `0d06283` |
| #8 / D-2 | 哈希伪向量静默入库 → 未配置即拒绝；降级一律留标记；概览暴露降级占比 | `bfc0414` |
| #10 / A-4 | 契约漂移无法机器校验 → 新增覆盖率脚本 + 两条 CI 测试；查出 2 条「文档说 A、实现没有 A」 | `0ecc0e9` |
| D-4 | 非 1024 维向量静默丢弃 → 按维度计数并显式上报 | `a56fd08` |
| S-2 | review-opinions 无 handler 层节点范围校验 → 补齐，实测越范围 403 | `a56fd08` |
| S-4 | X-User-Id 可伪造进审计 → 认证开启时只认登录态 | `a56fd08` |
| F-4 | 400ms 轮询与后端 0.4s 节流共振 → SSE 兜底路径放宽到 1.5s | `a56fd08` |
| #9 / A-1 | 内存常驻状态不可度量 → 管理端暴露 stateFootprint（实测 148 MB / 17870 条） | `492b36c` |

### 未动（都是工程专项，非缺陷）

A-2（routes.py 30k 行巨石 + repo.state 双口径）、A-3（分层倒置与循环导入）、
A-5（14 个 compose 口径不唯一）、R-8（Temporal 重试无步骤级断点）、
D-3（主链路词法检索 vs 语义检索）、F-2（Workbench.vue 9231 行）、
F-3（主链路 e2e 缺失）、#17（条款绑定标签，业务方决定保持现状）。

每一项都是独立的重构/基建专项，不适合混在缺陷修复批次里做。

### 遗留

**服务器部署通道仍然断着**：`dev-bjy` 不在 docker 组、无免密 sudo、`~/AIcheck`
不存在、全机没有 AIcheck 服务在监听。本轮全部改动都只推到 `origin/main`，
线上跑的仍是旧代码。所有「在线确认」指的是本地运行的真实服务（uvicorn + 真实
登录态 + 真实素材），不是生产环境。
