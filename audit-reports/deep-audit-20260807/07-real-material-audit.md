...
## admin / FDE 角色与跨项目隔离专项（本轮无新发现）

前几轮 P1 集中在 `owner`，故专门走了权限最大的 admin、FDE 两个角色，以及跨项目边界。
**结论是干净的**，记录于此以免后续重复审：
...
| 操作 | 结果 |
|---|---|
| 保存审查意见 | 403「角色 admin 无权执行 review:save」|
| 打回资料 | 403 |
| 事实修正 | 403 |
| 挂载资料 | 403 |
| 触发 AI 复核 | 403 |

职责分离到位——系统管理员不能替监检人员下结论，这对监检业务的法律效力是必要的。

### FDE 与业务面完全隔离

FDE 对项目维度的读写**全部 403**（「用户未获得该项目授权」），
靠项目成员资格而非动作矩阵拦截，属更强的隔离方式。

FDE 自己的治理面正常可用且边界清晰：

| 治理端点 | FDE | 其他角色 |
|---|---|---|
| `/fde/ai-runs` | 20 条 | inspection/contractor/owner 均 **403** |
| `/fde/ocr-runs` | 20 条 | — |
| `/fde/feedback` | 3 条 | — |

且治理面读到的 AI 运行**已裁剪敏感字段**（`rawOcrText` 不存在、`evidenceLinks` 为空），
符合「FDE 只管 AI 能力与治理流程，不碰业务证据」的定位。
...
|---|---|
| inspection / admin | 14 |
| contractor / ndt / owner | 3 |

以施工方身份直接访问其不可见的项目 `P-2025-CQARCH-007`：

```
项目详情 / 资料台账 / 节点树 / 报告  → 全部 403 FORBIDDEN
```

**未发现跨项目越权**。这与前几轮的角色内读放开（M-9/M-11）形成对比：
项目边界是硬的，项目内的角色边界是软的。
