...
| atomicCheck | 审核内容 | requiredFacts | tools | parameters | outputSchema | 状态 |
|---|---|---|---|---|---|---|
| AC-R39-01 | 工作见证：需提供相关单项无损检测工艺文件、操作指导书 | `r39.documentContent`<br>`r39.approvalChain`<br>`r39.firstUseValidation` | `get_document_ocr_result`<br>`extract_document_fields`<br>`extract_table_records`<br>`evaluate_r39_document_content`<br>`evaluate_r39_approval_chain`<br>`evaluate_r39_first_use_validation`<br>`validate_evidence_grounding` | `profile=ndt_procedure_partial_review`<br>`clauseSource=frozen_standard_clause_package`<br>`failurePolicy=business_rule_result`<br>`pendingCapabilities=['procedure_reference_consistency', 'method_specific_technical_requirements', 'complete_document_and_application_inventory']` | `deterministic-tool-result-v1` | `binding_only` |
