...
| ----------------------------------------------------------- | ------------------------------------------------------------------ | -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------ |
| `GET /api/workbench/projects`                               | `role` 必填；`keyword/status/page/pageSize` 可选                   | 无                                                                         | `Page<Project & { revision: number; etag: string }>`，每项必须含 `todoCount/messageCount/currentNodeId/actions` | 无                                         |
| `POST /api/admin/projects`                                  | Header `Idempotency-Key` 可选但生产建议必填                        | `AdminProjectCreatePayload`                                                | `AdminProjectCreateResult`，`project/detail.project` 带 `revision/etag`                       | 新建项目、69 个节点、初始成员授权，写审计  |
| `POST /api/projects`                                        | Header `Idempotency-Key` 可选但生产建议必填                        | `AdminProjectCreatePayload`                                                | `AdminProjectCreateResult`，`project/detail.project` 带 `revision/etag`                       | 兼容立项入口，幂等代理到后台立项实现       |
| `GET /api/projects/{projectId}`                             | `projectId` 必填                                                   | 无                                                                         | `AdminProjectDetailPayload`，`project` 带 `revision/etag`                                     | 无                                         |
| `PATCH /api/projects/{projectId}`                           | `projectId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `Partial<Project>`                                                         | `MockMutationResult & { project: Project & { revision: number; etag: string } }`              | 更新项目基础信息、递增版本并写审计         |
| `GET /api/projects/{projectId}/participants`                 | `projectId` 必填                                                   | 无                                                                         | `ProjectUnit[]`                                                                                | 无                                         |
| `POST /api/projects/{projectId}/participants`                | `projectId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `Partial<ProjectUnit>`                                                     | `MockMutationResult & { project: Project & { revision: number; etag: string } }`              | 保存参建单位、递增项目版本并写审计         |
| `PATCH /api/projects/{projectId}/participants/{participantId}` | `projectId/participantId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `Partial<ProjectUnit>`                                                     | `MockMutationResult & { project: Project & { revision: number; etag: string } }`              | 更新参建单位、递增项目版本并写审计         |
| `GET /api/projects/{projectId}/members`                     | `projectId` 必填；`role/page/pageSize` 可选                        | 无                                                                         | `Page<ProjectMember & { revision: number; etag: string }>`                                    | 无                                         |
| `POST /api/projects/{projectId}/members`                    | `projectId` 必填；Header `Idempotency-Key` 可选但生产建议必填      | `ProjectMemberSavePayload`                                                 | `{ member: ProjectMember & { revision: number; etag: string }; auditLogId: string }`          | 新增或覆盖成员授权，递增项目版本并写审计   |
| `PUT/PATCH /api/projects/{projectId}/members/{memberId}`    | `projectId`、`memberId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `Partial<ProjectMemberSavePayload> & { status?: ProjectMember["status"] }` | `{ member: ProjectMember & { revision: number; etag: string }; auditLogId: string; changed: Array<{ field: string; before?: unknown; after?: unknown }> }` | 更新成员角色、节点范围、动作或状态，递增成员/项目版本并写审计 |
| `POST /api/projects/{projectId}/initialize-workflow`        | `projectId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ templateVersion?: string }`                                             | `MockMutationResult & { createdNodeCount: number; project: Project & { revision: number; etag: string } }` | 初始化 69 节点流程、递增项目版本并写审计   |
...
| `POST /api/projects/{projectId}/documents/upload-session`                      | `projectId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填             | `{ files: Array<{ fileName: string; fileSize: number; fileType: string }>; sourceOrgId?: string; defaultUsage?: string; defaultNodeIds?: number[]; remark?: string }`                   | `{ uploadSessionId: string; uploadUrls: Array<SignedUrl & { fileName: string; documentId: string; documentVersionId: string }>; expiresAt: string }` | 新增 `DocumentAsset/DocumentVersion`；当前前端 mock 可直接置为 `已上传`，真实联调按 `complete` 确认 |
| `POST /api/projects/{projectId}/documents/upload-session/{sessionId}/complete` | `projectId`、`sessionId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ completedFiles: Array<{ documentVersionId: string; hash?: string; fileSize?: number }> }`                                                                                            | `MockMutationResult & { queuedTasks: DispatchResult[]; fileCount: number }`                                                                          | 文件变为 `已上传`，创建 OCR 任务；幂等重放不重复投递                                                |
| `POST /api/projects/{projectId}/documents/{documentId}/versions`               | `projectId`、`documentId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ uploadSessionId?: string; replaceCurrent?: boolean; mode?: "append" 或 "replace"; remark?: string; fileSize?: number }`                                      | `MockMutationResult & { version: DocumentVersion }`                                                                                                  | 当前版本切换；幂等重放不重复创建版本                                                                |
| `POST /api/projects/{projectId}/documents/bindings`                            | `projectId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填             | `{ nodeId?: number; nodeIds?: number[]; bindings: Array<{ documentId: string; documentVersionId?: string; requirementId?: string; usage: NodeFileBinding['usage']; remark?: string }> }` | `MockMutationResult`，`changed` 按节点返回 `nodes.{nodeId}.status`                                                                                   | 新增或更新挂载，支持跨节点批量挂载，节点进度变化                                                    |
| `GET /api/projects/{projectId}/documents/bindings`                             | `projectId` 必填；`documentId/nodeId/status` 可选                                 | 无                                                                                                                                                                                      | `Page<NodeFileBinding>`                                                                                                                              | 无                                                                                                  |
| `PATCH /api/projects/{projectId}/documents/bindings/{bindingId}`               | `projectId`、`bindingId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ requirementId?: string; requirementName?: string; usage?: string; bindingStatus?: string; remark?: string }`                                                                         | `MockMutationResult & { binding: NodeFileBinding }`                                                                                                  | 更新挂载字段；幂等重放返回同一结果                                                                  |
| `DELETE /api/projects/{projectId}/documents/bindings/{bindingId}`              | `projectId`、`bindingId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ reason?: string }`                                                                                                                                                                   | `MockMutationResult & { binding: NodeFileBinding }`                                                                                                  | 草稿挂载解除；幂等重放不重复删除                                                                    |
| `POST /api/projects/{projectId}/documents/{documentId}/withdraw`               | `projectId`、`documentId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ reason?: string }`                                                                                                                                                                   | `MockMutationResult & { document: DocumentAsset }`                                                                                                   | 未提交文件变为 `已撤回`                                                                             |
| `POST /api/projects/{projectId}/documents/{documentId}/void`                   | `projectId`、`documentId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ reason?: string }`                                                                                                                                                                   | `MockMutationResult & { document: DocumentAsset }`                                                                                                   | 文件变为 `已作废`                                                                                   |
| `POST /api/projects/{projectId}/documents/batch-classify`                      | `projectId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填             | `{ documentVersionIds?: string[]; strategy?: "filename" 或 "ocr" 或 "ai" }`                                                                                                              | `{ suggestions: Array<{ documentId: string; fileName: string; suggestedNodeIds: number[]; confidence: number }>; auditLogId: string }`               | 无，前端确认后再绑定                                                                                |
...
| `POST /api/projects/{projectId}/inspection/nodes/{nodeId}/ai-recheck`                           | `projectId`、`nodeId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ documentVersionIds?: string[]; reason?: string; force?: boolean }`                                                                    | `{ runId: string; status: '排队中'                                                                                                                                                                                                                                                                                                                                                                             | '推理中'                                                                             | '完成'; estimatedSeconds?: number; latestRun?: AiReviewRun }`                                | 创建 AI run          |
...
| `POST /api/projects/{projectId}/inspection/nodes/{nodeId}/review-opinions`                      | `projectId`、`nodeId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ result: ReviewOpinion['result']; opinion: string; basis?: string; riskLevel?: string; evidenceLinkIds: string[]; aiRunId?: string }`  | `{ opinion: ReviewOpinion; nextStatus: '已通过'                                                                                                                                                                                                                                                                                                                                                                | '需补正'                                                                             | '不适用'; createdTodos?: TodoItem[] }`                                                       | 保存人工结论         |
| `GET /api/projects/{projectId}/inspection/nodes/{nodeId}/review-opinions`                       | `projectId`、`nodeId` 必填                              | 无                                                                                                                                       | `Page<ReviewOpinion>`                                                                                                                                                                                                                                                                                                                                                                                          | 无                                                                                   |
| `POST /api/projects/{projectId}/inspection/nodes/{nodeId}/ai-suggestions/{suggestionId}/adopt`  | `projectId`、`nodeId`、`suggestionId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ result: string; opinion: string; reason: string }`                                                                                    | `{ draftOpinion: ReviewOpinion; auditLogId: string }`                                                                                                                                                                                                                                                                                                                                                          | 仅生成草稿，不改变最终节点状态                                                       |
| `POST /api/projects/{projectId}/inspection/nodes/{nodeId}/ai-suggestions/{suggestionId}/reject` | `projectId`、`nodeId`、`suggestionId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ reason: string; manualOpinion?: string }`                                                                                             | `MockMutationResult`                                                                                                                                                                                                                                                                                                                                                                                           | AI 建议标记驳回                                                                      |
| `POST /api/projects/{projectId}/inspection/nodes/{nodeId}/actions/return-correction`            | `projectId`、`nodeId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ responsibleOrgId: string; deadline: string; requirement: string; evidenceLinkIds?: string[]; bindingIds?: string[] }`                 | `{ rectification: RectificationPayload; nextStatus: '退回补正中'; createdTodos: TodoItem[]; messages: MessageItem[] }`                                                                                                                                                                                                                                                                                         | 节点和绑定变为需补正                                                                 |
| `POST /api/projects/{projectId}/inspection/nodes/{nodeId}/report-review`                        | `projectId`、`nodeId` 必填；Header `If-Match/Idempotency-Key` 可选但生产建议必填 | `{ includeEvidence: boolean; reportScope?: 'currentNode'                                                                                 | 'project' }`                                                                                                                                                                                                                                                                                                                                                                                                   | `{ report: ReportVersion; nextStatus: '报告生成/复核中'; createdTodos: TodoItem[] }` | 创建报告草稿                                                                                 |
...
| `POST /api/projects/{projectId}/ndt/films`                           | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ filmNo: string; weldNo: string; method: string; testDate?: string }` | `{ film: NdtFilm }`                                                                      | 新增底片                   |
| `PATCH /api/projects/{projectId}/ndt/films/{filmId}`                 | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`Partial<NdtFilm>`                                | `{ film: NdtFilm }`                                                                      | 更新底片                   |
| `POST /api/projects/{projectId}/ndt/films/import`                    | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ fileId?: string; rows?: NdtFilm[] }`            | `{ imported: number; failed: Array<{ row: number; reason: string }>; films: NdtFilm[] }` | 批量新增                   |
| `GET /api/projects/{projectId}/ndt/records`                          | `filmId/reportId/sampleStatus/page/pageSize` 可选                                                       | `Page<NdtRecord>`                                                                        | 无                         |
| `POST /api/projects/{projectId}/ndt/records/import`                  | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ fileId?: string; nodeId: number; rows?: Partial<NdtRecord>[] }` | `NdtRecordImportPayload`                                                                 | 批量新增并生成导入消息     |
...
| `POST /api/projects/{projectId}/ndt/reports/upload-session`          | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ files: Array<{ fileName: string; fileSize: number; fileType: string }>; relatedFilmIds?: string[] }` | `{ uploadSessionId: string; uploadUrls: SignedUrl[] }`                                   | 新建报告文件；失败不写状态 |
| `POST /api/projects/{projectId}/ndt/submissions`                     | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ reportIds: string[]; filmIds?: string[]; nodeId: number }` | `{ submissionId: string; snapshotId: string; nextStatus: '待审查'; createdTodos: TodoItem[]; submittedReportIds: string[]; submittedFilmIds: string[] }`               | 检测资料提交并写入提交快照追溯               |
| `POST /api/projects/{projectId}/ndt/rectifications`                  | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ rectificationId: string; description: string; reportIds?: string[]; filmIds?: string[] }` | `{ rectification: RectificationPayload; nextStatus: '复审中' }`                          | 补正反馈                   |
...
| `GET /api/knowledge/sources`                                                           | `keyword/sourceType/status/page/pageSize` 可选                                                                                                                                                                       | `Page<KnowledgeSource & { revision: number; etag: string }>`                                                           | 无                                                                      |
| `POST /api/knowledge/sources`                                                          | Header `Idempotency-Key` 可选但生产建议必填；`KnowledgeSourceSavePayload`，`name` 必填                                                                                                                               | `{ source: KnowledgeSource & { revision: number; etag: string }; auditLogId: string }`                                 | 新增知识源，写审计                                                      |
| `GET /api/knowledge/sources/{sourceId}`                                                | `sourceId` 必填                                                                                                                                                                                                      | `{ source: KnowledgeSource & { revision: number; etag: string } }`                                                     | 无                                                                      |
| `PUT/PATCH /api/knowledge/sources/{sourceId}`                                          | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`Partial<KnowledgeSourceSavePayload>`                                                                                                                          | `{ source: KnowledgeSource & { revision: number; etag: string }; auditLogId: string; changed: Array<{ field: string; before?: unknown; after?: unknown }> }` | 更新知识源、递增版本、写审计；过期版本返回 `ETAG_CONFLICT`              |
| `POST /api/knowledge/sources/{sourceId}/enable`                                        | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ reason?: string }`                                                                                                                                          | `{ source: KnowledgeSource & { revision: number; etag: string }; auditLogId: string }`                                 | 状态变为 `启用`，递增版本并写审计                                       |
| `POST /api/knowledge/sources/{sourceId}/disable`                                       | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ reason?: string }`                                                                                                                                          | `{ source: KnowledgeSource & { revision: number; etag: string }; auditLogId: string }`                                 | 状态变为 `停用`，递增版本并写审计                                       |
...
| `GET /api/knowledge/tasks`                                                             | `taskType/status/page/pageSize` 可选                                                                                                                                                                                 | `Page<KnowledgeTask & { revision: number; etag: string }>`                                                             | 无                                                                      |
| `POST /api/knowledge/tasks/{taskId}/retry`                                             | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ reason?: string }`                                                                                                                                          | `{ task: KnowledgeTask & { revision: number; etag: string }; dispatches?: DispatchResult[] }`                          | 重试投递任务、递增版本；过期版本返回 `ETAG_CONFLICT`                    |
| `POST /api/knowledge/tasks/{taskId}/cancel`                                            | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ reason?: string }`                                                                                                                                          | `{ task: KnowledgeTask & { revision: number; etag: string } }`                                                         | 取消排队任务、递增版本；过期版本返回 `ETAG_CONFLICT`                    |
| `POST /api/knowledge/files/{fileId}/reindex`                                           | Header `Idempotency-Key` 可选但生产建议必填；`{ force?: boolean }`                                                                                                                                                   | `{ task: KnowledgeTask & { revision: number; etag: string } }`                                                         | 创建重建索引任务                                                        |
| `POST /api/knowledge/reindex`                                                          | `{ scope: "all" \| "project" \| "source"; projectId?: string; sourceId?: string }`                                                                                                                                   | `{ taskIds: string[] }`                                                                                                | 批量创建索引任务                                                        |
| `GET /api/knowledge/config`                                                            | 无                                                                                                                                                                                                                   | `{ config: KnowledgeConfig & { revision: number; etag: string }; updatedAt: string; revision: number; etag: string }`  | 知识库配置单例带版本信息                                                |
| `PUT/PATCH /api/knowledge/config`                                                      | Header `If-Match/Idempotency-Key` 可选但生产建议必填；Body `Partial<KnowledgeConfig>`，忽略 `revision/etag/updatedAt`                                                                                                 | `{ config: KnowledgeConfig & { revision: number; etag: string }; updatedAt: string; revision: number; etag: string; auditLogId: string }` | 更新配置、递增版本、写审计；过期版本返回 `ETAG_CONFLICT`；幂等重放返回同一结果 |
...
| `GET /api/rules/versions`                                                              | `keyword/status/page/pageSize` 可选                                                                                                                                                                                  | `Page<KnowledgeRuleVersion & { revision: number; etag: string }>`                                                      | 无                                                                      |
| `GET /api/rules/versions/{versionId}/diff`                                             | `targetVersionId/targetVersion` 可选；为空时默认取同 `ruleKey` 的已发布版本或最近可比版本                                                                                                                            | `KnowledgeRuleVersionDiffPayload`，`base/target` 带 `revision/etag`                                                    | 无                                                                      |
| `POST /api/rules/versions/{versionId}/publish`                                         | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ reason: string; effectiveAt?: string }`                                                                                                                     | `MockMutationResult & { rule: KnowledgeRuleVersion & { revision: number; etag: string } }`                             | 当前版本变为 `已发布`、递增版本、写审计；过期版本返回 `ETAG_CONFLICT`   |
| `POST /api/rules/versions/{versionId}/rollback`                                        | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ reason: string; targetVersion: string }`                                                                                                                    | `MockMutationResult & { rule: KnowledgeRuleVersion & { revision: number; etag: string }; target: KnowledgeRuleVersion & { revision: number; etag: string } }` | 当前版本变为 `已回滚`、目标版本变为 `已发布`、写审计                    |
...
| 后台配置 `tree-nodes/node-role-mappings/org-units/users/roles/workflow-state-machines` | 列表 query；新增/编辑使用对应实体 `Partial<T>`；写接口支持 Header `If-Match/Idempotency-Key`                                                                                                                         | 列表统一 `Page<T>`；保存统一 `{ item: T; revision: number; etag: string }` 或 `MockMutationResult`                     | 配置状态变化，递增后台配置单例版本并写审计                              |
| `GET /api/admin/config-overview`                                                       | 无                                                                                                                                                                                                                   | `AdminConfigOverviewPayload & { revision: number; etag: string; updatedAt: string }`                                   | 后台配置总览单例带版本信息                                              |
...
| `POST /api/admin/config-items/{target}`                                                | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`target` 支持 `todo-rule/message-template/tool-source/field-mapping`；Body: `AdminConfigCreatePayload`                                                        | `AdminConfigSaveResult & { revision: number; etag: string }`                                                           | 新增细项配置，写审计并返回创建差异；过期版本返回 `ETAG_CONFLICT`；幂等重放返回同一结果 |
| `PUT /api/admin/config-items/{target}/{id}`                                            | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`target/id` 必填；Body: `AdminConfigChangePayload`                                                                                                             | `AdminConfigSaveResult & { revision: number; etag: string }`                                                           | 更新权限矩阵、节点模板、流程状态机或细项配置，写审计；过期版本返回 `ETAG_CONFLICT`；幂等重放返回同一结果 |
| `POST /api/admin/config-overview/publish`                                              | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ scope: "all" \| "permission" \| "workflow" \| "node-template" \| "rule"; reason: string }`                                                                 | `AdminPublishConfigPayload & { revision: number; etag: string }`                                                       | 生成配置版本、联动影响摘要和审计；过期版本返回 `ETAG_CONFLICT`          |
...
知识库配置和后台配置总览均按单例版本化资源处理。前端读取 `GET /api/knowledge/config` 或 `GET /api/admin/config-overview` 后，应在保存、发布等写接口携带当前 `etag` 到 `If-Match`，并为实际写入请求携带 `Idempotency-Key`；后端也接受 `If-Match: *` 用于受控脚本或首次迁移，过期版本统一返回 `ETAG_CONFLICT`。
...
| `GET /api/todos`                           | `role/projectId/status/page/pageSize` 可选                                                                                                                           | `Page<TodoItem & { revision: number; etag: string }>`                    | 无                         |
| `GET /api/todos/{todoId}`                  | `todoId` 必填                                                                                                                                                        | `TodoItem & { revision: number; etag: string; relatedObject?: unknown; evidenceLinks?: EvidenceLink[] }` | 无                         |
| `POST /api/todos/{todoId}/complete`        | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ result?: string; comment?: string }`                                                                        | `MockMutationResult & { todo: TodoItem & { revision: number; etag: string } }` | 待办关闭、递增版本；过期版本返回 `ETAG_CONFLICT` |
| `POST /api/todos/{todoId}/defer`           | Header `If-Match/Idempotency-Key` 可选但生产建议必填；`{ deferTo?: string; deferredUntil?: string; reason?: string }`                                                | `MockMutationResult & { todo: TodoItem & { revision: number; etag: string } }` | 待办延期、递增版本         |
| `GET /api/messages`                        | `projectId/read/page/pageSize` 可选                                                                                                                                  | `Page<MessageItem & { revision: number; etag: string }>`                 | 无                         |
| `POST /api/messages/{messageId}/read`      | Header `If-Match/Idempotency-Key` 可选但生产建议必填                                                                                                                 | `MockMutationResult & { message: MessageItem & { revision: number; etag: string } }` | 消息已读、递增版本         |
| `POST /api/messages/read-all`              | Header `Idempotency-Key` 可选但生产建议必填；批量条件写支持 `If-Match: *`；`{ projectId?: string }`                                                                  | `{ affectedCount: number; messages: Array<MessageItem & { revision: number; etag: string }>; auditLogId: string }` | 消息全部已读               |
...
| 动作                                       | 必填字段                                                           | 条件必填                                             | 最小成功返回                                                 |
| ------------------------------------------ | ------------------------------------------------------------------ | ---------------------------------------------------- | ------------------------------------------------------------ |
| 新建项目 `POST /api/projects`              | `Idempotency-Key`、`code/name/type/region/ownerOrgId/plannedStartDate/plannedEndDate` | `contractorOrgId/inspectionOrgId` 在初始化流程前必填 | `{ project: Project }`                                       |
...
| 项目成员授权                               | `userId/role/nodeScope/actions`                                    | `expiresAt` 可选；更新已有成员时传 `If-Match`        | `{ member, auditLogId }`，成员对象带 `revision/etag`         |
...
| 幂等覆盖 | 重复提交同一 `Idempotency-Key` 和同一请求体返回同一结果；同 key 不同请求体返回 `IDEMPOTENCY_KEY_CONFLICT` |
